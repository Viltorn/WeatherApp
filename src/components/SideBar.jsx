...
const SideBar = () => {
  ...
  const { activeCity, weatherData } = useContext(WeatherContext);

  const { day, month, weekDay } = weatherData.currentDate;

  const handleTooglePanelClick = (value) => {
    if (value === 'close') {
  ...
  return (
    <aside className="side-bar">
      <div className="side-bar__content">
        <SearchPanel searchPanel={isSearchOpen} closeBtn={handleTooglePanelClick} />
        <div className="side-bar__fixed-block">
          <div className="side-bar__buttons-block">
            <button onClick={() => handleTooglePanelClick('open')} type="button" id="show-panel" className="buttons side-bar__btn" aria-label="Поиск города">
              Поиск города
            </button>
